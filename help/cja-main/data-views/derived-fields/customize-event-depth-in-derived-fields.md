---
title: イベントの深度のカスタマイズ
description: Customer Journey Analyticsの派生フィールドでイベントの深さを設定し、ページの深さ、内部検索の深さなどを分析でカスタマイズする方法を説明します。
feature: Data Views
role: Admin, User
level: Intermediate
doc-type: Feature Video
last-substantial-update: 2025-08-21T00:00:00.000Z
jira: KT-18790
thumbnail: 3471041.jpeg
exl-id: 98eef481-259e-4729-945c-8701f1216954
TQID: https://experienceleague.adobe.com/OtgzfjJdap7pVV4dIx66GW-U0aY1oIE-SHJn094CdWE
product_v2:
  - id: e98b7246-966c-4318-9e95-cad2f7a17dc7
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2:
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: fb8bcbdd846b74e46321c69b4ccee3752cbea5d4
workflow-type: tm+mt
source-wordcount: 84
ht-degree: 28%

---

# イベントの深度のカスタマイズ

Customer Journey Analyticsの派生フィールドでイベントの深さを設定して、ページの深さ、内部検索の深さなどの分析をカスタマイズする方法を説明します。 この機能により、ユーザーはユーザーの行動を正確に追跡し、データ主導のインサイトを強化できます。

詳しくは、[ドキュメント](https://experienceleague.adobe.com/ja/docs/analytics-platform/using/cja-dataviews/derived-fields){target="_blank"}を参照してください。

>[!VIDEO](https://video.tv.adobe.com/v/3471041/?learn=on)
