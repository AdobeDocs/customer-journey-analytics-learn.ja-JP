---
title: タイプキャスティングの使用
description: Customer Journey Analyticsの派生フィールドでタイプキャストを検索します。
feature: Data Views
role: Admin, User
level: Intermediate
doc-type: Feature Video
last-substantial-update: 2025-08-21T00:00:00.000Z
jira: KT-18791
thumbnail: 3471042.jpeg
exl-id: d7393a83-01e1-4d4a-a979-0c1e60a25617
TQID: https://experienceleague.adobe.com/sdfrveUTMvd7SHV5Yh0KpqMjnCORUJDWEqIFkgk-Axg
product_v2:
  - id: e98b7246-966c-4318-9e95-cad2f7a17dc7
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2:
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: fb8bcbdd846b74e46321c69b4ccee3752cbea5d4
workflow-type: tm+mt
source-wordcount: 68
ht-degree: 32%

---

# タイプキャスティングの使用

Customer Journey Analyticsの派生フィールドでタイプキャストを確認します。 タイプキャストは、高度な分析のために文字列を数値や日時に変換するなど、スキーマフィールドのデータタイプを調整します。 フィールドを簡単に設定したり組み合わせたりして、ビジネスの強力なインサイトを引き出します。

詳しくは、[ドキュメント](https://experienceleague.adobe.com/ja/docs/analytics-platform/using/cja-dataviews/derived-fields){target="_blank"}を参照してください。

>[!VIDEO](https://video.tv.adobe.com/v/3471042/?learn=on)
