---
title: CJAとMarketoの統合（B2B インサイト向け）
description: Customer Journey AnalyticsをMarketoと統合して、売上高とキャンペーン効果に重点を置いた、B2B マーケティングインサイトを強化する方法を説明します。
feature: Basics
role: User
level: Intermediate
doc-type: Feature Video
duration: 252
last-substantial-update: 2025-12-04T00:00:00Z
jira: KT-19915
source-git-commit: afff13a5436d054e664fc8f41d08a2ebbfa6e532
workflow-type: tm+mt
source-wordcount: '82'
ht-degree: 0%

---


# Customer Journey AnalyticsとMarketoの統合（B2B インサイト向け）

Customer Journey AnalyticsとMarketoの統合により、B2B データ分析がどのように強化されるかを説明します。 マーケティングチャネルの貢献度、収益への影響、キャンペーンの有効性に焦点を当てます。 この統合により、高度なアトリビューションモデルとオーディエンスターゲティング戦略を使用して、マーケティング活動が、特に経営幹部の商談を促進する方法を理解できるようになります。

>[!VIDEO](https://video.tv.adobe.com/v/3477503/?captions=jpn&learn=on)
