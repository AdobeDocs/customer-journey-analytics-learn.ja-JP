---
title: Customer Journey Analytics - B2B editionの概要
description: Customer Journey Analytics B2B エディションがどのように課題を解決し、B2B 環境での優位性を確立しているかを説明します。
feature: Basics
role: Leader, User
level: Beginner
doc-type: Feature Video
duration: 0
last-substantial-update: 2025-10-01T00:00:00Z
jira: KT-19320
thumbnail: KT-19320.jpeg
source-git-commit: afff13a5436d054e664fc8f41d08a2ebbfa6e532
workflow-type: tm+mt
source-wordcount: '48'
ht-degree: 75%

---


# Customer Journey Analytics - B2B editionの概要

Customer Journey Analytics B2B エディションがどのように課題を解決し、B2B 環境での優位性を確立しているかを説明します。

>[!SLIDE](cja-b2b-overview)