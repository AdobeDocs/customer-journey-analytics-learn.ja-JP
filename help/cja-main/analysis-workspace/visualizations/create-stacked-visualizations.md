---
title: Analysis Workspace プロジェクトでの 100% 積み重ねビジュアライゼーションの作成
description: Customer Journey AnalyticsのAnalysis Workspace プロジェクトで、生の値ではなくパーセンテージを使用して、完全に積み重ねられたビジュアライゼーションを設定する方法を説明します。
feature: Visualizations
doc-type: technical video
thumbnail: 23131.jpg
kt: 13434
role: User
level: Beginner
last-substantial-update: 2026-02-09T00:00:00Z
exl-id: 47c482e4-91b2-4c0d-bd6d-719f20b91204
source-git-commit: 0e2ff3688878b5d2e8f3373350608c040333225f
workflow-type: tm+mt
source-wordcount: '70'
ht-degree: 0%

---

# Analysis Workspace プロジェクトでの 100% 積み重ねビジュアライゼーションの作成

Customer Journey AnalyticsのAnalysis Workspace プロジェクトで、生の値ではなくパーセンテージを使用して、完全に積み重ねられたビジュアライゼーションを設定する方法を説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3479696/?quality=12&learn=on)

[!UICONTROL &#x200B; 完全に積み重ねられた変数 &#x200B;] のビジュアライゼーションについて詳しくは、[&#x200B; ドキュメント &#x200B;](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/bar.html?lang=ja){target="_blank"} を参照してください。
