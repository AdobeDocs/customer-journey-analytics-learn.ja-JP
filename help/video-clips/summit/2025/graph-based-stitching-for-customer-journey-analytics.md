---
title: Customer Journey Analyticsのグラフベースの合成
description: グラフベースの合成により、複数のデータセットをシームレスに統合し、実用的なインサイトを得るために統合された顧客像を構築する方法を説明します。
doc-type: Short Video
duration: 200
index: false
hide: true
recommendations: noCatalog, noDisplay
exl-id: 6b2c7972-2e10-4ab6-b3b6-53120d8a3ca2
TQID: https://experienceleague.adobe.com/FU7sgjwLBEGjAQ7MMnXZXGVVhkBaODEdhhhsOJ8v95A
product_v2: id: e98b7246-966c-4318-9e95-cad2f7a17dc7
topic_v2: id: e1e0219c-f879-479f-8427-888ed2a6e9c2id: f4e6943a-c91a-4134-a2c7-f4f20cfff2f0
source-git-commit: 7819767ad88011dfada67e61cd0e3ce7f72b3ee5
workflow-type: tm+mt
source-wordcount: 52
ht-degree: 0%

---

# Customer Journey Analyticsのグラフベースの合成

グラフベースの合成により、複数のデータセットをシームレスに統合し、実用的なインサイトを得るために統合された顧客像を構築する方法を説明します。

<!-- 62_S112_3442459_199_graphbased-stitching-for-customer-journey-analytics -->
>[!VIDEO](https://video.tv.adobe.com/v/3458317/?learn=on&enablevpops=true)
