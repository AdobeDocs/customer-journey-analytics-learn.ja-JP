---
title: Data as a Product - CJAの導入を計画する
description: データパイプライン、アクティベーション、ダウンストリームのユースケースに焦点を当てて、Customer Journey Analyticsの導入においてデータを商品として扱う方法を理解します。
doc-type: Short Video
duration: 60
index: false
hide: true
recommendations: noCatalog, noDisplay
exl-id: ffbf29b1-be7a-4477-bc47-0f6fa4e6b944
TQID: https://experienceleague.adobe.com/caOdB8zDSJ0jMm1iuXUp-l0qhowrPwbSmX4qoEOkYeg
product_v2:
  - id: e98b7246-966c-4318-9e95-cad2f7a17dc7
topic_v2:
  - id: addc3a3a-2b1c-4fdf-aea4-4b1eb2931ba6
  - id: b5ce8718-c3af-4fdb-a1a9-fca32f83a87c
source-git-commit: 1c8f07e670190a3d0e5b6b9d982157afcf6ec1d3
workflow-type: tm+mt
source-wordcount: 62
ht-degree: 0%

---

# Data as a Product:CJAの導入計画

データパイプライン、アクティベーション、ダウンストリームのユースケースに焦点を当てて、Customer Journey Analyticsの導入においてデータを商品として扱う方法を理解します。

<!-- 62_S113_3442460_59_data-as-a-product-planning-your-cja-implementation -->
>[!VIDEO](https://video.tv.adobe.com/v/3458332/?learn=on&enablevpops=true)

